...
public static class GradientDescent {
     ...
     */
    public static float Run(int env_id, AST ast, Vector3 target, List<AST> var_asts) {
        var in_vals = var_asts.ConvertAll(a => (Vector3) a.vals[env_id]);       // original input vals.
        ...
        for (int iter = 1; iter <= MAX_ITERS; iter++) {
            ...
            for (int i_w = 0; i_w < var_asts.Count; i_w++) {
                ...
                var eval = (Vector3) ast.vals[env_id];                          // f(w_z + delta_w_z)
                var delta_out = target - eval;                              // out - f(w_z + delta_w_z)
                var err_out = Vector3.SqrMagnitude(delta_out);              // |out - f(w_z + delta_w_z)|^2

                var fp_w_x = ((Derivative.FV) ast.Diff(env_id, ast_w, 0)).v;     // f'(w_x + delta_w_x)
                var fp_w_y = ((Derivative.FV) ast.Diff(env_id, ast_w, 1)).v;     // f'(w_y + delta_y_x)
                var fp_w_z = ((Derivative.FV) ast.Diff(env_id, ast_w, 2)).v;     // f'(w_z + delta_z_x)
                ...
                ast_w.vals[env_id] = w + new_delta_w;                           // update w_ast using new_delta_w.
                ast.ReEval(env_id);                                             // re-eval ast to maintain cache consistency.

                var new_eval = (Vector3) ast.vals[env_id];                      // f(w_z + new_delta_w_z)
                ...
                if (derr > 0) {                                             // error increased:
                    ast_w.vals[env_id] = w + delta_w;                           //      - revert w_ast update
                    ast.ReEval(env_id);                                         //        and re-establish cache consistency.
        ...
        var h_err = GetError(env_id, ast, target, in_deltas);

        // restore env and ast.
        for (int i = 0; i < var_asts.Count; i++) {
            var_asts[i].vals[env_id] = in_vals[i];
        }
        ast.ReEval(env_id);
    ...
    public static float GetError(int env_id, AST ast, Vector3 target, List<Vector3> deltas) {
        var eval = (Vector3) ast.vals[env_id];
